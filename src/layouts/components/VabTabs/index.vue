<template>
  <div id="tabs-container" class="tabs-container">
    <el-tabs v-model="tabActive" class="tabs-content" type="card" @tab-click="handleTabClick" @tab-remove="handleTabRemove">
      <!-- 至少显示一个首页标签 -->
      <el-tab-pane v-if="visitedRoutes.length === 0" :name="'/index'" :closable="false">
        <template slot="label">
          <vab-icon :icon="['fas', 'home']" class="tab-icon" />
          <span>首页</span>
        </template>
      </el-tab-pane>
      <!-- 正常渲染访问过的路由标签 -->
...
<script>
  ...
  export default {
      ...
      $route: {
        handler(route) {
          ...
          if (this.visitedRoutes.length > 0) {
            ...
            this.tabActive = tabActive || this.visitedRoutes[0].path
          } else {
    ...
    mounted() {
      // 调试信息
      console.log('TabsBar组件挂载，tabsBar状态:', this.tabsBar);
      console.log('当前访问路由:', this.visitedRoutes);
    },
    computed: {
      ...mapGetters({
        visitedRoutes: 'tabsBar/visitedRoutes',
